...
@discord.app_commands.describe(
    indices="GPS point index/indices (e.g., 34 or 25-35 or 25,27,33)",
    name="New name for the GPS point(s) (max 32 chars)",
    color="Hex color code (e.g., #FF75C9F1, #RRGGBB, or #AARRGGBB)"
)
async def revise_gps(
    ...
    if name is not None and len(name) > 32:
        ...
        return
    if color is not None:
        # Validate hex color format (supports #RGB, #RRGGBB, #AARRGGBB)
        color = color.upper()
        if not color.startswith('#'):
            await interaction.response.send_message(f"Color must start with # (e.g., #FF75C9F1). Got: {color}", ephemeral=True)
            return
        hex_part = color[1:]
        if len(hex_part) not in (3, 6, 8) or not all(c in '0123456789ABCDEF' for c in hex_part):
            await interaction.response.send_message(f"Invalid hex color format. Use #RGB, #RRGGBB, or #AARRGGBB. Got: {color}", ephemeral=True)
            return
    try:
        parsed = parse_index_string(indices)
    except ValueError as e:
